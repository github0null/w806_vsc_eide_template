...
if [ $code_encrypt -eq 1 ]
 then
  ...
  openssl rsautl -encrypt -in ${ToolsFolder}/W806/ca/key.txt -inkey ${ToolsFolder}/W806/ca/capub_"$prikey_sel".pem -pubin -out key_en.dat
  cat ${BinFolder}/"$ProjName"_enc.bin key_en.dat > ${BinFolder}/"$ProjName"_enc_key.bin
  cat ${BinFolder}/"$ProjName"_enc_key.bin ${ToolsFolder}/W806/ca/capub_"$prikey_sel"_N.dat > ${BinFolder}/"$ProjName"_enc_key_N.bin  
  ${ToolsFolder}/W806/wm_tool.exe -b ${BinFolder}/"$ProjName"_enc_key_N.bin -o ${BinFolder}/"$ProjName" -it $img_type -fc 0 -ra $run_img_pos -ih $run_img_header -ua $upd_img_pos -nh 0  -un 0
 else
  ${ToolsFolder}/W806/wm_tool.exe -b ${BinFolder}/"$ProjName".bin -o ${BinFolder}/"$ProjName" -it $img_type -fc 0 -ra $run_img_pos -ih $run_img_header -ua $upd_img_pos -nh 0  -un 0
fi

if [ $signature -eq 1 ]
 then
  openssl dgst -sign  ${ToolsFolder}/W806/ca/cakey.pem -sha1 -out ${BinFolder}/"$ProjName"_sign.dat ${BinFolder}/"$ProjName".img
  ...
  ${ToolsFolder}/W806/wm_tool.exe -b ${ToolsFolder}/W806/W806_secboot.bin -o ${BinFolder}/"$ProjName"_secboot -it 0 -fc 0 -ra $sec_img_pos -ih $sec_img_header -ua $upd_img_pos -nh $run_img_header  -un 0    
...
if [ $zip_type -eq 1 ]
 then
  if [ $signature -eq 1 ]
   then
    ${ToolsFolder}/W806/wm_tool.exe -b ${BinFolder}/"$ProjName"_sign.img -o ${BinFolder}/"$ProjName"_sign -it $img_type -fc 1 -ra $run_img_pos -ih $run_img_header -ua $upd_img_pos -nh 0  -un 0
    mv ${BinFolder}/"$ProjName"_sign_gz.img ${BinFolder}/"$ProjName"_sign_ota.img
  else
   ${ToolsFolder}/W806/wm_tool.exe -b ${BinFolder}/"$ProjName".img -o ${BinFolder}/"$ProjName" -it $img_type -fc 1 -ra $run_img_pos -ih $run_img_header -ua $upd_img_pos -nh 0  -un 0
